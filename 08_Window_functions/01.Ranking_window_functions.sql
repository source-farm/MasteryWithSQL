-- Оконные функции позволяют произвести некие вычисления над некоторой группой
-- строк таблицы (окно), которые каким-то образом связаны с текущей строкой. В
-- некотором смысле они похожи на связанные подзапросы (correlated subqueries).
-- От агрегатных функций они отличаются тем, что не схлопывают группу в одну
-- строку; каждая строка сохраняет свою идентичность.
